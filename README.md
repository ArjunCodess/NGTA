...
**TL;DR:** NGTA turns a clinical transformer from a black box into a glass box. It estimates epistemic uncertainty with MC Dropout, converts that uncertainty into NARS truth values, injects explicit human-written medical rules at inference time, and feeds the revised confidence back into attention so brittle evidence is downweighted before the final prediction is made.
...
- **Scale & Safety:** Benchmarked on `91,713` ICU stays and achieved the best reported Expected Calibration Error in this repository evaluation at `0.00583`, with a best Brier score of `0.05647`.
...
- On `wids`, all Transformer variants were very close on AUC, with the best AUC at `0.8803`. The key result there was calibration: the NARS-gated version achieved the best ECE at `0.00583` and the best Brier score at `0.05647`.
