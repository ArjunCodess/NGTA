...
\begin{abstract}
Clinical predictors can be overconfident when hospital data are sparse, missing, or shifted across sites. We present dynamic evidential routing, a neurosymbolic transformer interface that converts Monte Carlo dropout uncertainty into Non-Axiomatic Reasoning System (NARS)-style truth values, revises those values with explicit clinical rules, and feeds the resulting confidence signal back into attention. The implementation is intentionally a NAL truth-value interface, not a full NARS cognitive architecture with concept memory, task scheduling, or explicit evidential-base bookkeeping. We evaluate TCGA-THCA lymph node metastasis prediction as a clinical-genomic proof of concept and WiDS ICU mortality prediction as the primary high-missingness validation. On WiDS, the baseline transformer is marginally best on AUC, flat-confidence gating is best on expected calibration error, MC-confidence-only gating is best on accuracy, and the NARS-gated model is best on Brier score. Paired bootstrap intervals for the main within-transformer calibration comparisons include zero, so symbolic revision is not yet statistically isolated as the cause of calibration behavior. The positive result is narrower but useful for NeSy: the symbolic path is operational, auditable, and active at scale, firing 13,031 times across 8,551 of 13,757 held-out ICU stays while preserving competitive discrimination and calibration.
\end{abstract}

\section{Introduction}
\label{sec:intro}

Clinical AI systems often fail in a way that is not captured by discrimination alone: they can emit sharp probabilities when the available evidence is weak. Missing laboratory values, irregular measurement, and cohort shift are common in hospital data, yet standard neural predictors typically collapse all evidence into a single risk score \citep{ma2019,ovadia2019,ghassemi2021}. In high-stakes clinical settings, an overconfident wrong prediction is harder to defer, audit, or correct than a visibly uncertain one.

Transformers are flexible function approximators for clinical prediction \citep{rasmy2021,azizi2021,li2019}, but attention weights are not themselves evidential confidences. They do not indicate when a feature was trusted weakly, when a prediction was unstable under epistemic uncertainty, or when an explicit medical rule should revise the learned signal. This motivates a neurosymbolic interface in which neural predictions remain useful while symbolic evidence remains visible and revisable.

We use NARS two-dimensional truth values, where a statement has frequency $f$ and confidence $c$ \citep{wang1995,wang2006,wang2013}. Frequency describes the balance of positive and negative evidence; confidence describes the amount of evidential support. This representation is well matched to clinical prediction under missingness because it separates what the model predicts from how strongly the available evidence supports that prediction.

The contribution is a bidirectional interface between a transformer and NAL truth-value operations. First, a neural predictive distribution is mapped to an initial truth value. Second, explicit clinical rules revise feature-level truths in the same $(f,c)$ space. Third, revised confidence controls a normalized attention gate. The claim is deliberately narrow: dynamic evidential routing is an operational and auditable neurosymbolic mechanism for clinical inference under missingness, not a blanket claim that symbolic gating improves AUC on every benchmark.
