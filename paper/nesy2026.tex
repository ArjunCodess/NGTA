...
On TCGA-THCA, the transformer variants remain tightly clustered. Flat-confidence has the highest AUC at 0.7328 and the lowest Brier score at 0.2109, while NARS-gated routing has AUC 0.7311 and Brier score 0.2109 at the reported precision. The small 69-case test split does not statistically separate transformer variants, so TCGA is best read as a multimodal feasibility test rather than as evidence of symbolic-gating superiority.
...
WiDS provides the clearer scale test. All transformer variants have higher AUC than the random forest in this run. Within the transformer family, the baseline has the highest AUC, NARS-gated has the lowest Brier score and ECE at the reported precision, and MC-confidence-only has the highest accuracy. The differences among transformer variants are very small, so symbolic revision is active but not statistically isolated from generic confidence gating in this run.
...
Figure~\ref{fig:rule_firing} shows the rule-firing distribution. TCGA exercises the clinical-genomic rule path on a small held-out split, while WiDS shows frequent rule activation in a larger high-missingness cohort.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/figure-3-rule-firing.png}
\caption{Rule-trigger counts on held-out cases. TCGA-THCA uses a small clinical-genomic rule base; WiDS ICU shows frequent rule activation across four critical-care signals.}
\label{fig:rule_firing}
\end{figure}
...
On TCGA, a gamma sweep over $\gamma\in\{0.25,0.5,1,2,4\}$ showed small sensitivity to gating strength. NARS-gated AUC was 0.7336 for $\gamma\in\{0.25,0.5,1\}$, matched the baseline at $\gamma=2$, and dipped to 0.7303 at $\gamma=4$. Brier scores stayed within a narrow range from 0.21091 to 0.21104, all close to the baseline value of 0.21132. No gamma setting yields a clear statistical separation on this small split, so the sweep is a sensitivity result rather than an optimizable superiority regime.
...
Dynamic evidential routing converts neural uncertainty into an explicit NAL-style confidence variable, revises that variable with symbolic rules, and feeds it back into transformer attention. Across TCGA-THCA and WiDS ICU, the interface is operational across modalities and active at scale. The strongest current claim is that it provides an auditable inference-time interface for uncertainty-aware clinical prediction under missingness, not that symbolic gating is a standalone source of higher AUC or better calibration. Future work should test stronger uncertainty estimators, broader rule bases, external cohorts, and ablations that more sharply separate neural uncertainty gating from symbolic revision.
...
\iffalse
Final-version acknowledgement placeholder:
The authors thank Prof. Leilani H. Gilpin for detailed feedback on the paper's framing, NAL/NARS boundary, audit-trace presentation, rule-base discussion, and claims about evidence and calibration.
\fi
